...
TH1F* DividTGraphs(TGraphAsymmErrors* gr1, TGraphAsymmErrors* gr2){
    ...
    for (int k = 0; k < 2; ++k){
	    ...
	    if(num_y_hi > 0.015){
	      num_y_hi = (gr[k]->GetErrorYhigh(i-1) + gr[k]->GetErrorYhigh(i+1))/2;
	    }

            num_y_low = gr[k]->GetErrorYlow(i);

	    if(num_y_low > 0.015){
	      num_y_low = (gr[k]->GetErrorYlow(i-1) + gr[k]->GetErrorYlow(i+1))/2;
	    }
...
int make_ratioplots(TString _file, TString _canvas, TString _path1, TString _path2, TString _output, TString _legtext){
    ...
    // TCanvas* c1;
    // if(_file.Contains("pt")){
    //   c1 = (TCanvas*) f1->Get("tpTree/TightCut_NUM_tightID_all_DEN_genTracks_PAR_pt/fit_eff_plots/pt_PLOT");
    // }
    // else if(_file.Contains("eta")){
    //   c1 = (TCanvas*) f1->Get("tpTree/TightCut_NUM_tightID_all_DEN_genTracks_PAR_eta/fit_eff_plots/eta_PLOT");
    // }
    TGraphAsymmErrors* eff1 = (TGraphAsymmErrors*)c1->GetPrimitive("hxy_fit_eff");

    // TCanvas* c2;
    // if(_file.Contains("pt")){
    //   c2 = (TCanvas*) f2->Get("tpTree/TightCut_NUM_tightID_all_DEN_genTracks_PAR_pt/fit_eff_plots/pt_PLOT");
    // }
    // else if(_file.Contains("eta")){
    //   c2 = (TCanvas*) f2->Get("tpTree/TightCut_NUM_tightID_all_DEN_genTracks_PAR_eta/fit_eff_plots/eta_PLOT");
    // }
    ...
    if(_path1.Contains("DATA") && _path2.Contains("DATA")){
	    // _leg1 = "DATA 2018ABC (n-1 Tight ID)";
            // _leg2 = "DATA 2018ABC (Standard Tight ID)";
	    // _leg1 = "DATA 2017BCDEF (n-1 Tight ID)";
            // _leg2 = "DATA 2018ABC (Standard Tight ID)";
            _leg1 = "DATA 2017";
	    _leg2 = "DATA 2016 Legacy Rereco";

            if(_path2.Contains("_default")){_leg2 = "default";}
	    else if(_path2.Contains("_CMSshape")){ _leg2 = "CMSshape";}
    }
    if(_path1.Contains("MC") && _path2.Contains("MC")){
            _leg1 = "MC";
	    _leg2 = "MC/3";
            if(_path2.Contains("_default")){_leg2 = "default";}
	    else if(_path2.Contains("_CMSshape")){ _leg2 = "CMSshape";}
    ...
    //ratio->GetYaxis()->SetRangeUser(0.97,1.03);
